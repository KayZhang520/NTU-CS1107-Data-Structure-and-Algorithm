...
int HashInsert(HashTable* Q1Ptr, int key)
{
    ...
    int hcode = Hash(key, Q1Ptr->hSize);
	ListNode *head = Q1Ptr->Table[hcode].head;
	while(head != NULL){// first thing first check if key already exists
		if(head->key==key)return 0;
		head = head->next;
	}

    ListNode *newnode = malloc(sizeof(ListNode));//had to use malloc as non-ptr declaration end up having the same address every call.
    newnode->key = key;
    newnode->next = NULL;
    newnode->pre = NULL;
    if(Q1Ptr->Table[hcode].head != NULL){
		Q1Ptr->Table[hcode].head->pre = newnode;
		newnode->next = Q1Ptr->Table[hcode].head;
    }
    Q1Ptr->Table[hcode].head = newnode;
}
...
int HashDelete(HashTable* Q1Ptr, int key)
{
	...
	int hcode = Hash(key, Q1Ptr->hSize);
	ListNode *head = Q1Ptr->Table[hcode].head;
	if(key == head->key){//if the 1st node is the searched node
		Q1Ptr->Table[hcode].head = head->next;
		Q1Ptr->Table[hcode].head->pre = NULL;
		free(head);
		return 1;
	}
	while(head != NULL){
		if(key==head->key){
			if(head->next == NULL){//if last node is the searched node
				head->pre->next = NULL;
				free(head);
				return 1;
			}
			else{//if middle nodes has the searched node
